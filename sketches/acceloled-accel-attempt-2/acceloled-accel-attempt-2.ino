...
uint8_t blueAspect(uint32_t color) {
  return (uint8_t)(color & 0xFF);
}

uint8_t redAspect(uint32_t color) {
  return (uint8_t)(color >> 16 & 0xFF);
}

uint8_t greenAspect(uint32_t color) {
  return (uint8_t)(color >> 8 & 0xFF);
}

uint8_t brightnessModifiedAspect(uint8_t aspect) {
  return (uint8_t)(((uint32_t)aspect) * brightness_ / 10);
}

uint32_t brightnessAdjustedColor(uint32_t c) {
  uint32_t adjusted = color(brightnessModifiedAspect(redAspect(c)),
                            brightnessModifiedAspect(greenAspect(c)),
                            brightnessModifiedAspect(blueAspect(c)));
  /*
...
void updateByAccelerometer() {
  if (time_ - last_accel_read_ >= BNO055_SAMPLERATE_DELAY_MS) {
    ...
    if (pattern_ == SPARKLE_ACCEL) {
      printAccelerometerState();
      brightness_ = max(1, min(10, (linear_acceleration_magnitude_ / 2)));
